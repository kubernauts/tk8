...
func (p EKS) Setup(args []string) {
	...
	WorkerNodeSetStdout, _ := WorkerNodeSet.StdoutPipe()
	...
	WorkerNodeSetScanner := bufio.NewScanner(WorkerNodeSetStdout)
	for WorkerNodeSetScanner.Scan() {
		m := WorkerNodeSetScanner.Text()
		fmt.Println(m)
	}
	WorkerNodeSet.Wait()
